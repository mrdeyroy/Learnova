...
import { getCompensation } from "@/lib/compensationRegistry";
    ...
    await db.collection("pending_operations").updateOne(
      { operationId },
      {
        $set: {
          ...
          status: "in_progress",
          steps: steps.map((s) => {
            const step = { name: s.name, status: "pending" };
            if (s.compensateKey) step.compensateKey = s.compensateKey;
            return step;
          }),
          updatedAt: new Date(),
        },
...
export async function executeSaga({
  ...
  try {
    ...
    try {
      await updatePendingOperation(operationId, "failed", {
        error: sagaError.message,
        // Unexpected saga errors may leave committed steps behind; flag the
        // record so the reconciliation job / an admin knows to review it.
        needs_review: true,
      });
    } catch (updateError) {
      logger.error(
        "[TransactionCoordinator] Failed to mark saga failed after unexpected error:",
        {
          operationId,
...
/**
 * Resolves a stale operation that was stuck in "in_progress" (or compensating
 * without full compensation). Marks it `failed` with a `needs_review` flag so a
 * deterministic idempotency key is released (retries no longer 409), and
 * dispatches registered compensation for any steps that were committed before
 * the crash.
 *
 * @param {Object} op - The stale pending-operation document
 * @returns {Promise<Object>} Outcome of the reconciliation
 */
export async function reconcileStuckOperation(op) {
  const db = await connectDb();
  const compensationResults = [];
  let allCompensated = true;

  for (const step of op.steps || []) {
    if (step.status !== "completed" || !step.compensateKey) continue;

    const handler = getCompensation(op.operationType, step.compensateKey);
    if (!handler) {
    ...
    const succeeded = await executeCompensation(
      () => handler({ operationId: op.operationId, op }),
      `${op.operationType}:${step.compensateKey}`,
      op.operationId
    );
    compensationResults.push({ step: step.name, succeeded });
    if (!succeeded) allCompensated = false;
  }
