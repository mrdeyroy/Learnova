...
// ============================================================================
/**
 * Sentinel used as the per-IP rate-limit key component when no valid public
 * client IP can be resolved. A fixed sentinel ensures spoofed/missing headers
 * cannot rotate a bucket and cannot collapse distinct clients together.
 */
export const RATE_LIMIT_IP_FALLBACK = "rate-limit-no-ip";

/**
 * Reads a header value from either the Fetch `Headers` API (used by Next.js
 * route handlers) or a plain object (used by middleware/tests).
 */
 ...
 * - Otherwise takes the rightmost validated hop from x-forwarded-for
 *   (appended by the last proxy); the leftmost hops are client-controlled
 *   and are never trusted.
 * - Rejects private/loopback/reserved IPs to prevent spoofing.
 * - Returns null (never the raw header) when no valid public IP is found.
 */
